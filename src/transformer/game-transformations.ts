		...
		gamesThisWeek.forEach((game, i, arr) => {
			...
			if (!game.stats.Kicking) {
				...
				if (receptionsIndex >= 0) {
					playerStatsByStatColumn.receptions[receptionsIndex].stat += parseInt(game.stats.Receiving?.receptions ?? "0");
				} else {
			...
			// defense
			if (game.stats.Defense && game.stats.Defense.ydsAllowed && game.stats.Defense.ptsAllowed) {
...
class StatColumns {
	...
	sortAllStatsColumns() {
		...
		this.int.sort((p1, p2) => p2.stat - p1.stat);
		...
		this.ydsAllowed.sort((p1, p2) => p2.stat - p1.stat);
		this.ptsAllowed.sort((p1, p2) => p2.stat - p1.stat);
	}
}
